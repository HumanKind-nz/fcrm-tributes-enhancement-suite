...
namespace FcrmEnhancementSuite;
 ...
 *
 * Compatible with standard WP-Cron and server-level (WP-CLI based) cron.
 *
...
class Instant_Indexing {
	...
	public static function init() {
		...
		register_deactivation_hook(FCRM_ENHANCEMENT_SUITE_FILE, [__CLASS__, 'deactivate']);
	}
	...
	private static function is_google_indexing_enabled() {
		return (bool) get_setting('indexing_google_enabled', false);
	}
	...
	private static function is_indexnow_enabled() {
		return (bool) get_setting('indexing_indexnow_enabled', false);
	}
	...
	public static function submit_urls($urls) {
		...
		if (self::is_google_indexing_enabled()) {
			...
			$daily_limit = (int) get_setting('indexing_google_quota', 200);
	...
	private static function submit_to_google($urls) {
		$credentials = get_setting('indexing_google_credentials', '');
	...
	public static function render_settings() {
		$google_enabled = get_setting('indexing_google_enabled', false);
		$google_credentials = get_setting('indexing_google_credentials', '');
		$google_quota = get_setting('indexing_google_quota', 200);
		$indexnow_enabled = get_setting('indexing_indexnow_enabled', false);
		...
		<div class="settings-section">
			...
			<div class="section-content">
				...
				<div class="notice notice-info inline" style="margin-bottom: 15px;">
					<p><strong>How it works:</strong> The plugin checks for new tributes hourly via WP-Cron (standard or server-level) and submits new URLs to Google and Bing/Yandex for immediate indexing.</p>
				</div>
	...
	private static function log_debug($message) {
		$debug_enabled = (bool) get_setting('debug_logging', false);
